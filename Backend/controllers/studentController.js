...
export const respondToInvite = asyncHandler(async (req, res, next) => {
    ...
    if (invite.status !== "Pending") {
        return next(new ErrorHandler(`Invite is already ${invite.status}`, 400));
    }

    if (invite.email.toLowerCase() !== req.user.email.toLowerCase()) {
        return next(new ErrorHandler("This invite is not for you", 403));
    }
    ...
    if (action === "accept") {
        ...
        if (!project) return next(new ErrorHandler("Project no longer exists", 404));

        // Ensure user isn't already in another project
        ...
        return res.status(200).json({ success: true, message: "Successfully joined the project" });
    }
