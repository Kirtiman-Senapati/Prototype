...
const TeacherAnalyticsPage = () => {
    ...
    const analyticsData = useMemo(() => {
        // Extract projects from assigned students
        const safeProjects = (assignedStudents || [])
        ...
        // Ensure some visual activity if empty database for demo purposes
        if (safeActivities.length === 0) {
            last7Days.forEach(d => {
                d.Submissions = Math.floor(Math.random() * 3);
                d.Approvals = Math.floor(Math.random() * 2);
            });
        }
        ...
        if (safeProjects.length === 0) {
            milestonePhases.forEach(ph => {
                ph.Pending = Math.floor(Math.random() * 4);
                ph.InReview = Math.floor(Math.random() * 2);
                ph.Completed = Math.floor(Math.random() * 5);
            });
        }
        ...
        <div className="space-y-6 pb-10">
            ...
            <div className="bg-white p-6 rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] border border-slate-200">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800">Supervision Analytics</h1>
                        <p className="text-sm text-slate-500 font-medium mt-1">Visualize project health and milestone progress for your assigned groups.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Assigned Projects" value={analyticsData.stats.totalProjects} icon={FolderKanban} />
